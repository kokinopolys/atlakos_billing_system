...
export default function EditCotizacion() {
  ...
  useEffect(() => {
    Promise.all([api.getCotizacion(id), api.getConfig()])
      .then(([cot, cfg]) => {
        if (cot.error) { setLoadError(cot.error); return }
        if (cot.status === 'facturada') {
          navigate(`/cotizacion/${id}`, { replace: true })
          return
        }
        setConfig(cfg)
        ...
        setVentaExonerada(cot.venta_exonerada ? String(cot.venta_exonerada) : '')
        setSubtotalExento(cot.subtotal_exento ? String(cot.subtotal_exento) : '')
        setDescuentos(cot.descuentos ? String(cot.descuentos) : '')
        setNoOcExenta(cot.no_oc_exenta || '')
        setNoRegistroExonerado(cot.no_registro_exonerado || '')
        setNoRegistroSag(cot.no_registro_sag || '')
      })
  ...
  const previewInvoice = {
    invoice_number: invoiceNumber || 'COT-000001',
    doc_type: 'cotizacion',
    date,
    ...
    return (
      <div className="p-12 text-center text-red-500">
        ...
        <button
          ...
          className="mt-4 inline-block px-4 py-2 bg-blue-700 text-white rounded-lg text-sm"
        >
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="w-1/2 overflow-y-auto p-5 border-r border-gray-200 bg-white">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900">Editando Cotización</h1>
          {invoiceNumber && (
            <p className="text-sm font-mono text-amber-700 mt-0.5">{invoiceNumber}</p>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => navigate(`/cotizacion/${id}`)}
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex-1 py-3 bg-amber-600 hover:bg-amber-700 disabled:bg-amber-400 text-white font-bold text-sm rounded-xl transition-colors"
            >
