    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-bold text-gray-800">{entry ? 'Editar CXP' : 'Nueva Cuenta por Pagar'}</h2>
        ...
        <div className="p-6 space-y-3">
          ...
          <div>
            ...
            <input type="text" value={form.supplier_name} onChange={e => set('supplier_name', e.target.value)} placeholder="Nombre del proveedor"
              className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-red-400" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1">Referencia (Factura/Recibo)</label>
              <input type="text" value={form.reference} onChange={e => set('reference', e.target.value)} placeholder="No. de factura o recibo"
                className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-red-400" />
            </div>
            <div>
...
export default function CXP() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setResumen(sum || { pendiente: 0, pagada: 0, vencida: 0 })
    } catch { setEntries([]) }
  ...
  const handleSave = async (form) => {
    if (!form.supplier_name || !form.amount) return alert('Proveedor y monto son obligatorios')
    if (modal === 'new') await api.createCXP(form)
    else await api.updateCXP(selected._id, form)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="px-6 py-4 flex flex-wrap items-center justify-between gap-3" style={{ backgroundColor: ACCENT }}>
        <div>
          <h1 className="text-xl font-bold text-white">Cuentas por Pagar (CXP)</h1>
          <p className="text-red-100 text-sm">Registro de pagos pendientes y realizados</p>
        </div>
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-5">
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Por Pagar', value: resumen.pendiente, color: ACCENT },
            { label: 'Pagado', value: resumen.pagada, color: '#374151' },
            { label: 'Vencido', value: resumen.vencida, color: '#7C3AED' },
          ].map(c => (
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <input type="text" placeholder="Buscar proveedor, referencia..." value={search} onChange={e => setSearch(e.target.value)}
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm w-64 focus:outline-none focus:ring-1 focus:ring-red-400" />
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
            className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-red-400">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={e._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 font-semibold text-gray-900 whitespace-nowrap">{e.supplier_name || e.description || '—'}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {e.source === 'gasto'
                        ? <span className="text-xs px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 font-semibold">Gasto</span>
